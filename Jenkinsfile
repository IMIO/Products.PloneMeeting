        ...
        stage('Build') {
            steps {
                    ...
                    caches: [[$class: 'ArbitraryFileCache', excludes: '', path: '${WORKSPACE}/eggs']]){
                            sh 'sed -ie "s#PloneMeeting.git branch=master#PloneMeeting.git branch=${BRANCH_NAME}#" communes-dev.cfg'
                            sh 'make jenkins'
                }
                stash 'workspace'
            }
                ...
                stage('Test Coverage') {
                    steps {
                        dir('test_coverage') {
                            ...
                            sh 'bin/coverage run --source=Products.PloneMeeting bin/test --test=!robot'
