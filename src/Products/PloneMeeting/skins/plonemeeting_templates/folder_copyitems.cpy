    ...
    for id in ids:
        try:
            ...
            if not item.showCopyItemAction():
                msg = _(u"You can not copy this item : ${title}, your are not in the same proposing group (${proposingGroup}).", mapping={u'title':unicode(item.Title(), 'utf-8'), u'proposingGroup':context.displayValue(item.listProposingGroup(), item.getProposingGroup())})
                raise CopyError, msg
        except CopyError:
