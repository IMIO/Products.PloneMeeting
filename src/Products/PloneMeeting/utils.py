...
def sendMail(recipients, obj, event, attachments=None, mapping={}):
    ...
    if cfg.config_group and several_mc_with_same_title():
...
def get_enabled_ordered_wfas(tool):
    ...
    from Products.PloneMeeting.content.meeting_config import MeetingConfig
    return tuple(
        [wfa for wfa in MeetingConfig.wfAdaptations
         if wfa in itertools.chain.from_iterable(
             [cfg.workflow_adaptations for cfg in tool.objectValues('MeetingConfig')])])
