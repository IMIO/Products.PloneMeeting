...
class PloneMeetingTestCase(unittest.TestCase, PloneMeetingTestingHelpers):
    ...
    def _activate_wfas(self, wfas, cfg=None, keep_existing=False):
        ...
        if not keep_existing:
            ...
            wfas = tuple(set(tuple(wfas) + cfg.workflow_adaptations))
        if wfas:
            cfg.workflow_adaptations = wfas
            notify(ObjectEditedEvent(cfg))
        self.changeUser(currentUser)
    ...
    def _deactivate_wfas(self, wfas, cfg=None):
        ...
        if cfg is None:
            cfg = self.meetingConfig
        wfas = [wfa for wfa in cfg.workflow_adaptations
                if wfa not in wfas]
        cfg.workflow_adaptations = wfas
        notify(ObjectEditedEvent(cfg))
        self.changeUser(currentUser)
    ...
    def _activate_config(self,
        ...
        values = list(cfg._dx_field_value(field_name))
        if remove:
            values.remove(value)
        else:
            if keep_existing:
                ...
                values = [value]
        cfg._set_dx_field_value(field_name, values)
        if reload:
            currentUser = self.member.getId()
            self.changeUser('siteadmin')
            notify(ObjectEditedEvent(cfg))
    ...
    def _enableField(self, field_names, cfg=None, related_to='MeetingItem', enable=True, reload=False):
        ...
        for field_name in field_names:
            if related_to == 'MeetingItem':
                ...
                if field_name not in cfg._dx_field_vocabulary_values('usedItemAttributes'):
                ...
                # make sure we are not playing with a field_name that does not exist
                if field_name not in cfg._dx_field_vocabulary_values('usedMeetingAttributes'):
