...
class MeetingInsertingMethodsHelpMsgView(BrowserView):
    ...
    def renderConfigField(self, field_name):
        """Render a MeetingConfig DX field value for the insertion help popup."""
        value = self.cfg._dx_field_value(field_name)
        if isinstance(value, (list, tuple)):
            return u', '.join([safe_unicode(elem) for elem in value])
